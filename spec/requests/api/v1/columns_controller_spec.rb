...
RSpec.describe 'Api::V1::ColumnsController', type: :request do
  ...
  describe 'POST /api/v1/columns' do
    it 'creating column' do
      ...
      post "#{url}/#{board.id}/columns/", params: { title: 'Title Test', board_id: 1 }, headers: headers
      expect(response).to have_http_status(200)
    end
  ...
  describe 'PUT /api/v1/columns' do
    it 'updating column' do
      headers = { 'ACCEPT' => 'application/json' }
      put "#{url}/#{board.id}/columns/#{column.id}", params: { id: column.id, title: 'testupd', board_id: 1 },
                                                     headers: headers
      expect(response).to have_http_status(200)
    end
